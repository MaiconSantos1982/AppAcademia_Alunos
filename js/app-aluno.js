...
async function renderizarExercicios() {
  ...
  area.innerHTML = `<button class="btn btn-warning w-100 mb-4 fw-bold rounded-3 shadow-sm" style="font-size:1.12rem;" onclick="desmarcarTodos()">Desmarcar todos</button>`
    + await Promise.all(Object.entries(grupos).map(async ([letra, lista]) => {
      ...
      <div class="mb-4">
        ...
        ${await Promise.all(lista.map(async (ex) => {
          ...
          // Exercício expandido ou não
          const exercOpen = exercicioExpandido[ex.id] ?? false;

          // Resumo do exercício (visor fechado)
          return `
